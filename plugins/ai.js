...
malvin({
    ...
    category: "ai",
    react: "🤖",
    use: ".masky <your query>",
    filename: __filename,
}, async (conn, mek, m, { from, args, q, reply }) => {
    try {
        // Vérification de l'entrée utilisateur
        if (!q) return reply("⚠️ Please provide a query for MASKY Ai.\n\nExample:\n.masky What is AI?");
        ...
        if (!response || !response.data || !response.data.result) {
            return reply("❌ No response received from the Masky API. Please try again later.");
        }
        ...
        // Image AI à envoyer
        const ALIVE_IMG = 'https://files.catbox.moe/0gh0x5.png'; // Remplacez par l'URL de votre image AI

        // Légende avec des informations formatées
        const formattedInfo = `🤖 *Masky's Response:*\n\n${gptResponse}`;
...
malvin({
    ...
    try {
        ...
        // AI image to attach
        const AI_IMG = 'https://files.catbox.moe/w58wuv.png'; // Replace with a valid image URL
